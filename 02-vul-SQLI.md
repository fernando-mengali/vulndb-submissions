...
## **Vulnerable Endpoint:**  
- `/pizzafy/admin/ajax.php?action=delete_menu`
...
A **Error-based SQL Injection vulnerability** was discovered in the **DELETE functionality** of the **Pizzafy Ecommerce System 1.0**. This vulnerability occurs because the `id` parameter is not properly sanitized, allowing an attacker to inject malicious SQL commands into the backend database query.
...
function delete_menu(){
    ...
    if($delete) {
        ...
        error_log("Erro to delete product: " . $this->conn->error);
        return 0;
    }
}
```
